...
interface CupholderOptionsStepProps {
...
const CupholderOptionsStep: React.FC<CupholderOptionsStepProps> = ({ onSelect, onBack }) => {
  ...
  const options = [
    {
      id: 'chain-handle',
      name: 'Ручка-цепочка',
      description: 'Элегантная цепочка для удобного ношения подстаканника'
    },
    {
      id: 'short-handle',
      name: 'Короткая ручка',
      description: 'Компактная ручка для удобного использования'
    }
  ];
  ...
  const handleSubmit = () => {
    if (selectedOptions.length === 0) {
      message.warning('Пожалуйста, выберите хотя бы одну опцию');
      return;
    }
...
export default CupholderOptionsStep; 